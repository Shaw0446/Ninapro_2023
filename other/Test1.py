...
if __name__ == '__main__':
    file = h5py.File('../data/DB2_S1segment.h5', 'r')
    X_test = file['testData'][:]
    Y_test = file['testLabel'][:]

    Y_test = to_categorical(np.array(Y_test))
    model = keras.models.load_model('MutiCnn.h5')
    ...
    # multiclassEva(cm)
    plot_confusion_matrix(cm,'DB2_S1muti.png')
    classes = []
    for i in range(len(cm)):
        classes.append(str(i))
    # print(f1_score(Y_test.argmax(axis=1), Y_predict.argmax(axis=1), average='weighted'))
    # print(f1_score(Y_test.argmax(axis=1), Y_predict.argmax(axis=1), average='micro'))
